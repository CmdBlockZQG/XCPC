...
template <typename T> struct point {
  ...
  long double ang(const point &a) const {
    return acosl(max(-1.0, min(1.0, ((*this) * a) / (len() * a.len()))));
  } // 向量夹角
...
template <typename T> struct convex : polygon<T> {
  ...
  template <typename F> size_t extreme(const F &dir) const {
    ...
    const auto cmp = [&](const Point &v) {
      ...
      if (vi == 1 && checkv == check0 && dir0.toleft(v - p[0]) == 0)
        return 1;
...
bool segs_inter(const vector<Segment> &segs) {
  ...
  const auto seqcmp = [](const seq_t &u, const seq_t &v) {
    ...
    if (abs(u0 - v0) <= eps)
      return make_tuple(u1, u2.a, u2.b) < make_tuple(v1, v2.a, v2.b);
    return u0 < v0 - eps;
  };
  vector<seq_t> seq;
  for (auto seg : segs) {
